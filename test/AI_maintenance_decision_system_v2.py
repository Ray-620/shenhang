...
if prompt := st.chat_input():
    ...
    for i in range(len(msg['measure'])):
        if str(msg['measure_num'][i]) in handbook:
            ...
            Task.append(str(handbook[measure_num]))

    output_area = st.empty()
    text = '##### 检修建议\n\n'
    for task in Task:
        client = ZhipuAI(api_key="")  # 填写您自己的APIKey
        content = "根据我给出的内容返回一个详细检修推荐的中文步骤。必须使用提供的上下文来回答，不许自己给出解释!!!如果你不知道答案，就说你不知道，不要试图编造答案。给出的内容包括几个相似问题的历史检修操作，和检修手册中检修操作的上下文或只有历史检修操作。其中手册的内容上下文中可能有多个Task，或者一个Task对应多个SUBTASK+对应的编号，你需要自己去识别对应的Task对应的内容。请从上下文中选择相关的文本来回答我的输入问题，不相关的内容请忽略。prompt =开始吧!我的输入:{" + str(
            question) + "}，历史检修操作：{" + str(history) + "}给出的上下文:{" + str(
            task) + "}若有手册内容，请以以下格式回复您的回复:助理回答:Task-编号:对应的全部详细操作步骤。若没有手册内容，只需要根据历史检修记录整理出具体详细的操作步骤，不要提到我是否提供手册内容。"
